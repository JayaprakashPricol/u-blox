...
#define TLV_TYPE_CHAN_TRPC_CONFIG             0x0189
#define FIX_FIELD_TRPC_CMD                      12
#define ENABLE_TXPWRLIMIT_CHANNELS              2
...
typedef MLAN_PACK_START struct _MrvlIETypes_ChanTRPCConfig_t {
	...
	t_u8 chan_num;
    /** mod group data */
	t_u8 mod_group_data[0];
} MLAN_PACK_END MrvlIETypes_ChanTRPCConfig_t;

/**
 *  @brief parse the trpc command raw data and fill in chan_list
 *
 *  @param trpc_raw_data   trpc hostcmd raw data
 *  @param length          total length of trcp raw data
 *  @param chan_list       buffer pointer to save the channel list
 *  @param max_chan        max channel number support in chan_list array
 *  @param chan_idx        index in chan_list array
 *
 *  @return                -1 or chan_idx
 */
int
wlan_parse_trpc_raw_data(t_u8 *trpc_raw_data, t_u16 length, t_u8 *chan_list,
			 t_u8 max_chan, int *chan_idx)
{
	...
	pByte = trpc_raw_data + FIX_FIELD_TRPC_CMD;
	left_len = length - FIX_FIELD_TRPC_CMD;
	while (left_len >= sizeof(MrvlIEtypesHeader_t)) {
		if (idx >= max_chan) {
			PRINTM(MERROR, "chan_list buf too small, idx=%d\n",
			       idx);
			break;
		}
		pTlvHdr = (MrvlIEtypesHeader_t *)pByte;
		pTlvHdr->len = le16_to_cpu(pTlvHdr->len);

		switch (le16_to_cpu(pTlvHdr->type)) {
		case TLV_TYPE_CHAN_TRPC_CONFIG:
			trpc_tlv = (MrvlIETypes_ChanTRPCConfig_t *) pTlvHdr;
			chan_list[idx] = trpc_tlv->chan_num;
			idx++;
			break;
		default:
			break;
		}
		left_len -= (pTlvHdr->len + sizeof(MrvlIEtypesHeader_t));
		pByte += pTlvHdr->len + sizeof(MrvlIEtypesHeader_t);
	}
	*chan_idx = idx;
	return idx;
}
...
/**
 *    @brief WOAL parse ASCII format txpwrlimit raw data to hex format
 *
 *    @param handle       MOAL handle
 *    @param data         Source data
 *    @param size         data length
 ...
 */
static t_u32
woal_process_txpwrlimit_cfg(moal_handle *handle, t_u8 *data, t_size size)
{
	t_u8 *pos = data;
	t_u8 *intf_s, *intf_e;
	t_u8 *buf = NULL;
	...
	gfp_t flag;
	t_u8 chan_list[WLAN_USER_SCAN_CHAN_MAX];
	int chan_idx = 0;
	int ret = 0;
	...
	while ((pos - data) < size) {
		...
		if (*pos == '}') {
			...
			ret = wlan_parse_trpc_raw_data(buf, cmd_len, chan_list,
	...
	if (chan_idx) {
		handle->num_chan = chan_idx;
		memcpy(handle->chan_list, chan_list, chan_idx);
		PRINTM(MCMND, "Txpwrlimit file include total chan=%d\n",
		       chan_idx);
		DBG_HEXDUMP(MCMD_D, "chan list", chan_list, chan_idx);
	}
done:
	kfree(buf);
	LEAVE();
...
#if defined(STA_CFG80211) || defined(UAP_CFG80211)
			if (type == COUNTRY_POWER_TABLE &&
			    cntry_txpwr == ENABLE_TXPWRLIMIT_CHANNELS) {
				if (handle->wiphy &&
				    IS_STA_OR_UAP_CFG80211(cfg80211_wext))
					woal_process_txpwrlimit_cfg(handle,
								    cfg_data,
								    len);
			}
#endif
...
#if defined(STA_CFG80211) || defined(UAP_CFG80211)
			if ((cntry_txpwr == ENABLE_TXPWRLIMIT_CHANNELS) &&
			    IS_STA_OR_UAP_CFG80211(cfg80211_wext))
				woal_process_txpwrlimit_cfg(handle,
							    (t_u8 *)handle->
							    txpwr_data->data,
...
#if defined(STA_CFG80211) || defined(UAP_CFG80211)
	else if (handle->wiphy && IS_STA_OR_UAP_CFG80211(cfg80211_wext) &&
		 handle->num_chan) {
		woal_enable_channel_list(priv);
	}
#endif
	LEAVE();
	return ret;
...
MODULE_PARM_DESC(cntry_txpwr,
		 "Allow setting tx power table of country; 0: disable (default), 1: enable. 2:enable and only enable channels in tx power table file");
