...
/**
 * @brief  Enable channel list in wiphy
 *
 * @param priv          A pointer moal_private structure
 * @param chan          chan to search
 *
 * @return              0-not find, 1- find
 */
static t_u8
woal_find_chan(moal_private *priv, t_u8 chan)
{
	t_u8 find = 0;
	int i;
	for (i = 0; i < priv->phandle->num_chan; i++) {
		if (priv->phandle->chan_list[i] == chan) {
			find = 1;
			break;
		}
	}
 ...
 *
 * @param priv          A pointer moal_private structure
 * @param chan_list     A pointer to chan_list buffer
 * @param num_chan      number of channels in chan_list
 *
 * @return              N/A
 */
void
woal_enable_channel_list(moal_private *priv)
{
	...
	ENTER();
	if (priv && priv->phandle->wiphy)
		wiphy = priv->phandle->wiphy;
	if (!wiphy || !priv->phandle->num_chan) {
		LEAVE();
		return;
	}
	...
	if (sband) {
		for (i = 0; i < sband->n_channels; i++) {
			if (!woal_find_chan(priv, sband->channels[i].hw_value)) {
				PRINTM(MCMND, "Txpwr: Disable channel=%d\n",
				       sband->channels[i].hw_value);
				sband->channels[i].flags |=
					IEEE80211_CHAN_DISABLED;
			} else {
				PRINTM(MCMND, "Txpwr: Find channel=%d\n",
				       sband->channels[i].hw_value);
			}
		}
	}
