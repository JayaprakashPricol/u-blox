...
process_getcfgchanlist(int argc, char *argv[])
{
	...
	for (i = 0; i < chan_list->num_chan; i++) {
		if (chan_list->chan_list[i].flags & IEEE80211_CHAN_DISABLED)
			printf("        * %04d MHZ [%d] (disabled)\n",
			       chan_list->chan_list[i].center_freq,
			       chan_list->chan_list[i].hw_value);
		else if (chan_list->chan_list[i].flags & IEEE80211_CHAN_RADAR)
			printf("        * %04d MHZ [%d] (%d) dBm (DFS)\n",
			       chan_list->chan_list[i].center_freq,
			       chan_list->chan_list[i].hw_value,
			       chan_list->chan_list[i].max_power);
		else
			printf("        * %04d MHZ [%d] (%d) dBm\n",
			       chan_list->chan_list[i].center_freq,
			       chan_list->chan_list[i].hw_value,
			       chan_list->chan_list[i].max_power);
	}
